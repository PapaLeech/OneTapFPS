...
func _physics_process(delta):
	...
	if PresenceManager.scoreboard_open or Input.mouse_mode == Input.MOUSE_MODE_VISIBLE:
		_is_aiming = false
		if _camera and current_weapon:
			_camera.fov = lerp(_camera.fov, 75.0, current_weapon.ads_speed * delta)
		return
	...
	if _camera and not current_weapon.is_melee and _bullet_hole:
		...
		if multiplayer.has_multiplayer_peer() and not multiplayer.is_server():
			...
			if local_player and local_player.has_method("_server_shot"):
				local_player._server_shot.rpc_id(1, ray_origin, aim_dir, Time.get_ticks_usec() / 1_000_000.0, current_weapon.damage)
