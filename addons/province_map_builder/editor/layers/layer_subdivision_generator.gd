...
func open(province_map: ProvinceMap, child_layer: MapLayer) -> void:
...
func _on_apply() -> void:
	...
	# Sync cross-parent boundaries first: ensure adjacent child regions from different
	# parents share the same global vertex indices at their shared edge.
	# Without this, A-side and B-side children have independent indices at the same
	# boundary positions, so moving one tears the other.
	_sync_cross_parent_boundaries(new_regions, working_verts)

	# Enrich each parent region's polygon with child boundary intersection vertices.
	# After the sync above, both sides of every shared parent edge have the same child
	# vertex indices, so enrichment is now symmetric and won't cause tears.
	...
	# Cascade enrichment upward to grandparent and further ancestors.
	# A vertex newly inserted into _parent_layer's polygon (by the loop above) may itself
	# lie on an edge of a grandparent layer. Without cascading up, the grandparent never
	# learns about that vertex, so:
	#   (a) moving it won't propagate to the grandparent layer (visual gap), and
	#   (b) _is_on_map_boundary() returns false for it (wrong deletion permission).
