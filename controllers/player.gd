...
func _ready():
	...
	if multiplayer.has_multiplayer_peer() and not is_multiplayer_authority():
		...
		_target_rot_y = global_rotation.y
		set_physics_process(false)
		set_process_unhandled_input(false)
...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("jump") and is_on_floor() and not _chat_open and not _movement_frozen:
		velocity.y = JUMP_VELOCITY

	var input_dir := Vector2.ZERO if (_chat_open or _movement_frozen) else Input.get_vector("move_left", "move_right", "move_forward", "move_backward")
		...
		if multiplayer.has_multiplayer_peer() and not multiplayer.is_server():
			...
			if Engine.get_physics_frames() % 60 == 0:
				...
				if peer and peer is ENetMultiplayerPeer:
					var ping := float(peer.get_peer(1).get_statistic(ENetPacketPeer.PEER_ROUND_TRIP_TIME))
					if ping > 0:
...
func _process(delta: float) -> void:
	...
	current_rot.y = lerp_angle(current_rot.y, _target_rot_y, INTERP_SPEED * delta)
